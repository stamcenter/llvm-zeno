...
bool RegisterCoalescer::reMaterializeTrivialDef(const CoalescerPair &CP,
  ...
  if (DstReg.isVirtual()) {
    ...
    if (DefRC != nullptr) {
      const TargetRegisterClass* TempNewRC = nullptr;
      if (NewIdx)
        TempNewRC = TRI->getMatchingSuperRegClass(NewRC, DefRC, NewIdx);
      else
        TempNewRC = TRI->getCommonSubClass(NewRC, DefRC);
      // TODO(jacob-abraham) dirty hack for zeno
      if(!TempNewRC) {
        if(NewRC == TRI->getPointerRegClass(*MF))
          TempNewRC = NewRC;
      }

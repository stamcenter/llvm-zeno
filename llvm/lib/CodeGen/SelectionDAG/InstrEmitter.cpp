...
void InstrEmitter::CreateVirtualRegisters(SDNode *Node,
  ...
  for (unsigned i = 0; i < NumVRegs; ++i) {
    ...
    if (i < NumResults && TLI->isTypeLegal(Node->getSimpleValueType(i))) {
      ...
      // TODO(jacob-abraham) little bit hacky but works
      if(VTRC && RC && TRI->getRegSizeInBits(*VTRC) > TRI->getRegSizeInBits(*RC))
        RC = nullptr;
      if (RC)
...
EmitMachineNode(SDNode *Node, bool IsClone, bool IsCloned,
  ...
  // dbgs() << "num op " << NumMIOperands << " num op " << Node->getNumOperands() << " num reuslts " << NumResults << " II op " << II.getNumOperands() << " II impl " << II.getNumImplicitDefs() << " IMPUse " << NumImpUses << "\n";

  // for(auto _idx = 0; _idx < Node->getNumOperands(); _idx++) {
  //   Node->getOperand(_idx)->dump();
  // }

...
static void emitSCSPrologue(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  if(MF.getSubtarget<RISCVSubtarget>().hasStdExtZZeno()) {
    //StoreOpcode = IsRV64 ? RISCV::PseudoESD : RISCV::PseudoESW;
    StoreOpcode = IsRV64 ? RISCV::ESD : RISCV::ESW; // TODO(jacob-abraham) optimize for ptrs and fix ESW, maybe need new opcode for 32 bit ptrs
  }
  else {
    StoreOpcode = IsRV64 ? RISCV::SD : RISCV::SW;
  }

  BuildMI(MBB, MI, DL, TII->get(StoreOpcode))
      .addReg(RAReg)
      .addReg(SCSPReg)
      .addImm(0)
...
static void emitSCSEpilogue(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  if(MF.getSubtarget<RISCVSubtarget>().hasStdExtZZeno()) {
    // LoadOpcode = IsRV64 ? RISCV::PseudoELD : RISCV::PseudoELW;
    LoadOpcode = IsRV64 ? RISCV::ELD : RISCV::ELW; // TODO(jacob-abraham) optimize for ptrs and fix ESW, maybe need new opcode for 32 bit ptrs
  }
  else {
    LoadOpcode = IsRV64 ? RISCV::LD : RISCV::LW;
  }

  BuildMI(MBB, MI, DL, TII->get(IsRV64 ? LoadOpcode : LoadOpcode))
...
void RISCVFrameLowering::adjustReg(MachineBasicBlock &MBB,
  ...
  // set namespace for stack
  if (MBB.getParent()->getSubtarget<RISCVSubtarget>().hasStdExtZZeno() &&
      (DestReg == getSPReg(STI) || DestReg == getFPReg(STI))) {
    MCRegister DestExtReg = RISCV::E0 + (DestReg.id() - RISCV::X0);
    // BuildMI(MBB, MBBI, DL, TII->get(RISCV::EADDIE), DestExtReg)
...
// this logic is nearly identical to PrologEpilogInserter::assignCalleeSavedSpillSlots
// the extra klogic changes GPR and extended registers to PXER
// TODO(jacob-abraham) this could also be fixed by changing the calling conventions in RISCVCallingConv.td
bool RISCVFrameLowering::assignCalleeSavedSpillSlots(
  ...
  if (CSI.empty())
    return false; // Early exit if no callee saved registers are modified!
  ...
  for (auto &CS : CSI) {
    ...
    if(RC->hasSuperClassEq(&RISCV::PXERRegClass) || RC->hasSuperClassEq(&RISCV::GPRRegClass) || RC->hasSuperClassEq(&RISCV::ER64RegClass)) {
      RC = &RISCV::PXERRegClass;
    }

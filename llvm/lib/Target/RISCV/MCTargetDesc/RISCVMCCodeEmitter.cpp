...
public:
  ...
  void expandAddTPRel(const MCInst &MI, raw_ostream &OS,
                      SmallVectorImpl<MCFixup> &Fixups,
                      const MCSubtargetInfo &STI) const;
  // void expandExtendedLoad(const MCInst &MI, raw_ostream &OS,
  //                         SmallVectorImpl<MCFixup> &Fixups,
  //                         const MCSubtargetInfo &STI) const;
...
// // Expand Pseudo extended loads to real loads
// void RISCVMCCodeEmitter::expandExtendedLoad(const MCInst &MI, raw_ostream &OS,
//                                             SmallVectorImpl<MCFixup> &Fixups,
//                                             const MCSubtargetInfo &STI) const {
//   MCRegister DstGPR = MI.getOperand(0).getReg();
//   MCRegister SrcPseudo = MI.getOperand(1).getReg();
//   MCRegister SrcExt = RISCV::E0 + (SrcPseudo.id() - RISCV::PXE0);
//   int64_t Imm = MI.getOperand(2).getImm();

//   unsigned Opcode = 0;
//   switch(MI.getOpcode()) {
//     default: llvm_unreachable("Unimplemented pseudo instruction!");
//     case RISCV::PseudoELW: Opcode = RISCV::ELW; break;
//   }

//   // Emit a normal ADD instruction with the given operands.
//   MCInst TmpInst = MCInstBuilder(Opcode)
//                        .addReg(DstGPR)
//                        .addReg(SrcExt)
//                        .addImm(Imm);
//   uint32_t Binary = getBinaryCodeForInstr(TmpInst, Fixups, STI);
...
void RISCVMCCodeEmitter::encodeInstruction(const MCInst &MI, raw_ostream &OS,
  ...
  // if (MI.getOpcode() == RISCV::PseudoELW) {
  //   expandExtendedLoad(MI, OS, Fixups, STI);
  //   MCNumEmitted += 1;
  // }

...
void RISCVInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (RISCV::GPRRegClass.contains(SrcReg) && RISCV::PXERRegClass.contains(DstReg)) {
    unsigned id = DstReg.id() - RISCV::PXE0;
    MCRegister SrcEXTReg = RISCV::E0 + (SrcReg.id() - RISCV::X0);
    MCRegister DstEXTReg = RISCV::E0 + id;
    MCRegister DstGPRReg = RISCV::X0 + id;

    // MachineFunction *MF = MBB.getParent();
    // MachineRegisterInfo &MRI = MF->getRegInfo();
    // const TargetSubtargetInfo &STI = MF->getSubtarget();
    // const TargetRegisterInfo &TRI = *STI.getRegisterInfo();
    // dbgs() << llvm::printReg(DstReg, &TRI, 0, &MRI) << "\n";
    // dbgs() << llvm::printReg(RISCV::PXE0, &TRI, 0, &MRI) << "\n";
    // dbgs() << llvm::printReg(RISCV::X0, &TRI, 0, &MRI) << "\n";
    // dbgs() << llvm::printReg(DstEXTReg, &TRI, 0, &MRI) << "\n";
    // dbgs() << llvm::printReg(DstGPRReg, &TRI, 0, &MRI) << "\n";
    // dbgs() << DstReg.id() << " " << id << "\n" << "\n";

    // BuildMI(MBB, MBBI, DL, get(RISCV::EADDIE), DstEXTReg)
    //     .addReg(RISCV::X0) /* zero extended register */
    //     /*.addReg(SrcReg, getKillRegState(KillSrc))*/
    //     .addImm(0);
    BuildMI(MBB, MBBI, DL, get(RISCV::ADDI), DstGPRReg)
        .addReg(SrcReg, getKillRegState(KillSrc))
        .addImm(0);
    BuildMI(MBB, MBBI, DL, get(RISCV::EADDIX), DstEXTReg)
        .addReg(SrcEXTReg, getKillRegState(KillSrc))
        .addImm(0);
    return;
  }

  if (RISCV::PXERRegClass.contains(SrcReg, DstReg)) {
    unsigned dst_id = DstReg.id() - RISCV::PXE0;
    MCRegister DstEXTReg = RISCV::E0 + dst_id;
    MCRegister DstGPRReg = RISCV::X0 + dst_id;

    unsigned src_id = SrcReg.id() - RISCV::PXE0;
    MCRegister SrcEXTReg = RISCV::E0 + src_id;
    MCRegister SrcGPRReg = RISCV::X0 + src_id;

    BuildMI(MBB, MBBI, DL, get(RISCV::ADDI), DstGPRReg)
        .addReg(SrcGPRReg, getKillRegState(KillSrc))
        .addImm(0);
    BuildMI(MBB, MBBI, DL, get(RISCV::EADDIX), DstEXTReg)
        .addReg(SrcEXTReg, getKillRegState(KillSrc))
        .addImm(0);
  ...
  if (RISCV::PXERRegClass.contains(SrcReg) && RISCV::GPRRegClass.contains(DstReg)) {
    unsigned dst_id = DstReg.id() - RISCV::X0;
    MCRegister DstEXTReg = RISCV::E0 + dst_id;
    MCRegister DstGPRReg = RISCV::X0 + dst_id;
  ...
  if (RISCV::PXERRegClass.contains(SrcReg) && RISCV::ER64RegClass.contains(DstReg)) {
    unsigned dst_id = DstReg.id() - RISCV::E0;
    MCRegister DstEXTReg = RISCV::E0 + dst_id;
    MCRegister DstGPRReg = RISCV::X0 + dst_id;
  ...
  if (RISCV::GPRRegClass.contains(SrcReg) && RISCV::ER64RegClass.contains(DstReg)) {
    // "eaddie", "moveeb"
    BuildMI(MBB, MBBI, DL, get(RISCV::EADDIE), DstReg)
        .addReg(SrcReg, getKillRegState(KillSrc))
        .addImm(0);
    return;
  }

  if (RISCV::ER64RegClass.contains(SrcReg) && RISCV::GPRRegClass.contains(DstReg)) {
    // "eaddi", "movebe"
    BuildMI(MBB, MBBI, DL, get(RISCV::EADDI), DstReg)
        .addReg(SrcReg, getKillRegState(KillSrc))
        .addImm(0);
    return;
  }

  if (RISCV::ER64RegClass.contains(DstReg, SrcReg)) {
    BuildMI(MBB, MBBI, DL, get(RISCV::EADDIX), DstReg)
        .addReg(SrcReg, getKillRegState(KillSrc))
        .addImm(0);
    return;
  }
...
void RISCVInstrInfo::storeRegToStackSlot(MachineBasicBlock &MBB,
  ...
  unsigned Opcode;
  bool isZeno = MF->getSubtarget<RISCVSubtarget>().hasStdExtZZeno();
  bool IsScalableVector = true;
  bool IsZvlsseg = true;
  if (RISCV::GPRRegClass.hasSubClassEq(RC)) {
    if (isZeno) {
      Opcode = RISCV::PseudoESP;      
    } else {
      Opcode = TRI->getRegSizeInBits(RISCV::GPRRegClass) == 32 ? RISCV::SW
                                                               : RISCV::SD;
    }
    IsScalableVector = false;
  } else if(RISCV::ER64RegClass.hasSubClassEq(RC)) {
    Opcode = RISCV::PseudoESE;
    IsScalableVector = false;
  } else if(RISCV::PXERRegClass.hasSubClassEq(RC)) {
    Opcode = RISCV::PseudoESP;
    IsScalableVector = false;
...
bool RISCVInstrInfo::getRegSequenceLikeInputs(
  ...
  switch (MI.getOpcode()) {
  case RISCV::PseudoZenoCombinePtrNSID:
    // pxer = PseudoZenoCombinePtrNSID gpr, er
    // is the same as:
    // pxer = REG_SEQUENCE gpr, ssub_0, er, ssub_1
    // Populate the InputRegs accordingly.
    // gpr
    const MachineOperand *MOReg = &MI.getOperand(1);
    if (!MOReg->isUndef())
      InputRegs.push_back(RegSubRegPairAndIdx(MOReg->getReg(),
                                              MOReg->getSubReg(), RISCV::sub_ext_64));
    // er
    MOReg = &MI.getOperand(2);
    if (!MOReg->isUndef())
      InputRegs.push_back(RegSubRegPairAndIdx(MOReg->getReg(),
                                              MOReg->getSubReg(), RISCV::sub_ext_64_64));
    return true;
  }
  llvm_unreachable("Target dependent opcode missing");
}

bool RISCVInstrInfo::getExtractSubregLikeInputs(
    const MachineInstr &MI, unsigned DefIdx,
  ...
  switch (MI.getOpcode()) {
  case RISCV::PseudoZenoGetPtr:
  case RISCV::PseudoZenoGetNSID:
    ...
    InputReg.SubIdx = MI.getOpcode() == RISCV::PseudoZenoGetPtr ? RISCV::sub_ext_64 : RISCV::sub_ext_64_64;
    return true;
  }
  llvm_unreachable("Target dependent opcode missing");
}

bool RISCVInstrInfo::getInsertSubregLikeInputs(
  ...
  switch (MI.getOpcode()) {
  case RISCV::PseudoZenoSetPtr:
  case RISCV::PseudoZenoSetNSID:
    ...
    InsertedReg.SubIdx = MI.getOpcode() == RISCV::PseudoZenoSetPtr ? RISCV::sub_ext_64 : RISCV::sub_ext_64_64;
    return true;
  }
  llvm_unreachable("Target dependent opcode missing");
}
...
unsigned RISCVInstrInfo::getInstSizeInBytes(const MachineInstr &MI) const {
  ...
  switch (Opcode) {
  default: {
    // pseudo registers cause instructions 
    // to be doubled in size doubles instructions
    if(zeno::containsPseudoReg(MI)) return get(Opcode).getSize() * 2;
  ...
  case RISCV::PseudoTAIL:
    return 8;
  case RISCV::PseudoLAI:
  case RISCV::PseudoLA:
  case RISCV::PseudoLLA:
  case RISCV::PseudoLA_TLS_IE:
  case RISCV::PseudoLA_TLS_GD:
    if(ST.hasStdExtZZeno()) return 12;
    else return 8;
  ...
  case RISCV::PseudoESP:
  {
    return 8; // these emit an extra load/store
  }

  }
}

// TODO(jacob-abraham) add zeno to isAsCheapAsAMove and isCopyInstrImpl
...
RISCVInstrInfo::isCopyInstrImpl(const MachineInstr &MI) const {
  ...
  default:
    break;
  case RISCV::PseudoEADDI:
  case RISCV::PseudoEADDIE:
  case RISCV::PseudoEADDIX:
  case RISCV::EADDI:
  case RISCV::EADDIE:
  case RISCV::EADDIX:
  case RISCV::ADDI:

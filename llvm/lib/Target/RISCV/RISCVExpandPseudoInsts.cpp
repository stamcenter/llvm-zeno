...
private:
  ...
  bool expandLoadAddressImmediate(MachineBasicBlock &MBB,
                                  MachineBasicBlock::iterator MBBI,
                                  MachineBasicBlock::iterator &NextMBBI);
...
bool RISCVExpandPseudo::expandMI(MachineBasicBlock &MBB,
  ...
  case RISCV::PseudoLAI:
    return expandLoadAddressImmediate(MBB, MBBI, NextMBBI);
...
bool RISCVExpandPseudo::expandAuipcInstPair(
  ...
  if(MF->getSubtarget<RISCVSubtarget>().hasStdExtZZeno()) {
      BuildMI(NewMBB, DL, TII->get(RISCV::EADDIX), zeno::getExtendedReg(DestReg))
      .addReg(RISCV::E0)
      .addImm(0);
  }
...
bool RISCVExpandPseudo::expandLoadAddressImmediate(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    MachineBasicBlock::iterator &NextMBBI) {
  ...
  const MachineOperand &SymbolHi = MBBI->getOperand(1);
  const MachineOperand &SymbolLo = MBBI->getOperand(2);
  ...
  BuildMI(MBB, MBBI, DL, TII->get(RISCV::ADDI), DestReg)
      ...
      .addDisp(SymbolLo, 0);
  if(MF->getSubtarget<RISCVSubtarget>().hasStdExtZZeno()) {
      BuildMI(MBB, MBBI, DL, TII->get(RISCV::EADDIX), zeno::getExtendedReg(DestReg))
      .addReg(RISCV::E0)
      .addImm(0);
  }

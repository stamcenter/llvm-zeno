...
BitVector RISCVRegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  ...
  if (MF.getSubtarget<RISCVSubtarget>().hasStdExtZZeno()) {
    ...
    markSuperRegs(Reserved, zeno::getExtendedReg(RISCV::X0)); // zero
    markSuperRegs(Reserved, zeno::getPseudoReg(RISCV::X0));
    markSuperRegs(Reserved, zeno::getExtendedReg(RISCV::X2)); // sp
    markSuperRegs(Reserved, zeno::getPseudoReg(RISCV::X2));
    markSuperRegs(Reserved, zeno::getExtendedReg(RISCV::X3)); // gp
    markSuperRegs(Reserved, zeno::getPseudoReg(RISCV::X3));
    markSuperRegs(Reserved, zeno::getExtendedReg(RISCV::X4)); // tp
    markSuperRegs(Reserved, zeno::getPseudoReg(RISCV::X4));

    if (TFI->hasFP(MF)) {
      markSuperRegs(Reserved, zeno::getExtendedReg(RISCV::X8)); // fp
      markSuperRegs(Reserved, zeno::getPseudoReg(RISCV::X8));
    }
    // Reserve the base register if we need to realign the stack and allocate
    // variable-sized objects at runtime.
    if (TFI->hasBP(MF)) {
      markSuperRegs(Reserved, zeno::getExtendedReg(RISCVABI::getBPReg())); // bp
      markSuperRegs(Reserved, zeno::getPseudoReg(RISCVABI::getBPReg()));
    }
  }
...
bool RISCVRegisterInfo::isConstantPhysReg(MCRegister PhysReg) const {
  return PhysReg == RISCV::X0 || PhysReg == RISCV::E0 || PhysReg == RISCV::PXE0;
}

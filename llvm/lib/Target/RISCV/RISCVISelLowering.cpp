...
RISCVTargetLowering::RISCVTargetLowering(const TargetMachine &TM,
  ...
  if (Subtarget.hasStdExtZZeno()) {
    addRegisterClass(MVT::i128, &RISCV::PXERRegClass);
  }
  ...
  if (!Subtarget.hasStdExtM()) {
    ...
    if (Subtarget.is64Bit()) {
      ...
      // on zeno, 128 multiply must be legal
      if(!Subtarget.hasStdExtZZeno())
  ...
  // add zeno imstructions
  if (Subtarget.hasStdExtZZeno()) {
    // for(auto Ty: {MVT::i8, MVT::i16, MVT::i32, MVT::Other}) {
    //   setOperationAction(ISD::INTRINSIC_VOID, Ty, Custom);
    //   setOperationAction(ISD::INTRINSIC_W_CHAIN, Ty, Custom);
    // }
    // setOperationAction(ISD::INTRINSIC_VOID, MVT::Other, Custom);
    // setOperationAction(ISD::INTRINSIC_W_CHAIN, MVT::Other, Custom);

    // for(unsigned Opc = 0; Opc < ISD::BUILTIN_OP_END; Opc++) {
    //   setOperationAction(Opc, MVT::i128, Custom);
    // }
    ...
    setOperationAction(ISD::FrameIndex, MVT::i128, Legal);
    setOperationAction(ISD::GlobalAddress, MVT::i128, Custom);


    // setOperationAction(ISD::SETCC, MVT::i64, Custom);
    // setOperationAction(ISD::TargetConstant, MVT::i128, Custom);
    // setOperationAction(ISD::Constant, MVT::i128, Custom);
    // truncate to i64 from i128
    ...
    //i should just lower ISD::LOAD/STORE to a ZENO_LOAD/STORE, then i can do pattern matching:)
    // setTargetDAGCombine(ISD::TargetFrameIndex);
    setOperationAction(ISD::JumpTable, MVT::i128, Custom);


    // setOperationAction(ISD::ADD, MVT::i128, Legal);
    // setOperationAction(ISD::MUL, MVT::i128, Legal);
    // setOperationAction(ISD::SUB, MVT::i128, Legal);
    // setOperationAction(ISD::AND, MVT::i128, Legal);
    // setOperationAction(ISD::OR, MVT::i128, Legal);
    // setOperationAction(ISD::XOR, MVT::i128, Legal);
    // setOperationAction(ISD::SDIV, MVT::i128, Legal);
    // setOperationAction(ISD::SREM, MVT::i128, Legal);
    // setOperationAction(ISD::UDIV, MVT::i128, Legal);
    // setOperationAction(ISD::UREM, MVT::i128, Legal);
    // setOperationAction(ISD::SHL, MVT::i128, Legal);
    // setOperationAction(ISD::SRA, MVT::i128, Legal);
    // setOperationAction(ISD::SRL, MVT::i128, Legal);
...
EVT RISCVTargetLowering::getSetCCResultType(const DataLayout &DL,
                                            ...
                                            EVT VT) const {
  if(Subtarget.hasStdExtZZeno()) {
    if(VT.isSimple() && VT.getSimpleVT() == MVT::i128) return getPointerTy(DL);
    return MVT::i64;
  }
  if (!VT.isVector())
...
bool RISCVTargetLowering::getTgtMemIntrinsic(IntrinsicInfo &Info,
  ...
  default:
    return false;
  case Intrinsic::riscv_zeno_load_u8:
  case Intrinsic::riscv_zeno_load_u8_nsid: 
  case Intrinsic::riscv_zeno_load_s8:
  case Intrinsic::riscv_zeno_load_s8_nsid: 
  case Intrinsic::riscv_zeno_load_u16:
  case Intrinsic::riscv_zeno_load_u16_nsid: 
  case Intrinsic::riscv_zeno_load_s16:
  case Intrinsic::riscv_zeno_load_s16_nsid: 
  case Intrinsic::riscv_zeno_load_32:
  case Intrinsic::riscv_zeno_load_32_nsid:
  case Intrinsic::riscv_zeno_load_64:
  case Intrinsic::riscv_zeno_load_64_nsid:
  case Intrinsic::riscv_zeno_load_128:
  case Intrinsic::riscv_zeno_load_128_nsid: {
    PointerType *PtrTy = cast<PointerType>(I.getArgOperand(0)->getType());
    Info.opc = ISD::INTRINSIC_W_CHAIN;
    Info.memVT = MVT::getVT(PtrTy->getElementType());
    if (Intrinsic == Intrinsic::riscv_zeno_load_128_nsid ||
        Intrinsic == Intrinsic::riscv_zeno_load_128)
      Info.memVT = MVT::i128;
  ...
  case Intrinsic::riscv_zeno_store_8:
  case Intrinsic::riscv_zeno_store_8_nsid: 
  case Intrinsic::riscv_zeno_store_16:
  case Intrinsic::riscv_zeno_store_16_nsid: 
  case Intrinsic::riscv_zeno_store_32:
  case Intrinsic::riscv_zeno_store_32_nsid:
  case Intrinsic::riscv_zeno_store_64:
  case Intrinsic::riscv_zeno_store_64_nsid:
  case Intrinsic::riscv_zeno_store_128:
  case Intrinsic::riscv_zeno_store_128_nsid: {
    PointerType *PtrTy = cast<PointerType>(I.getArgOperand(0)->getType());
    Info.opc = ISD::INTRINSIC_W_CHAIN;
    Info.memVT = MVT::getVT(PtrTy->getElementType());
    Info.ptrVal = I.getArgOperand(0);
    if (Intrinsic == Intrinsic::riscv_zeno_store_128_nsid ||
        Intrinsic == Intrinsic::riscv_zeno_store_128)
      Info.memVT = MVT::i128;
    Info.offset = 0;
    Info.align.reset();
...
SDValue RISCVTargetLowering::LowerOperation(SDValue Op,
  ...
  case ISD::BR_CC: {
    ...
    return DAG.getNode(RISCVISD::BR_CC, DL, Op.getValueType(), Ops);
  }
  case ISD::TRUNCATE: {
    SDLoc DL(Op);
    MVT VT = Op.getSimpleValueType();

    if(VT == MVT::i64 && Op.getOperand(0).getSimpleValueType() == MVT::i128 && Subtarget.hasStdExtZZeno()) {
      return DAG.getNode(RISCVISD::ZENO_GET_PTR, DL, MVT::i64, Op.getOperand(0));
    }
...
// TODO(jacob-abraham) COME BACK AND FIX ME
...
SDValue RISCVTargetLowering::getAddr(NodeTy *N, SelectionDAG &DAG,
  ...
  LLVM_DEBUG(
    dbgs() << "Getting Address for isPIC[";
    switch(getTargetMachine().getRelocationModel()) {
      case Reloc::Static: dbgs() << "Static"; break;
      case Reloc::PIC_: dbgs() << "PIC"; break;
      case Reloc::DynamicNoPIC: dbgs() << "Dynamic NoPIC"; break;
      default: dbgs() << "Unknown";
    }
    dbgs() << "] isLocal[" << IsLocal <<"] CM[";
    switch(getTargetMachine().getCodeModel()) {
      case CodeModel::Tiny: dbgs() << "Tiny"; break;
      case CodeModel::Small: dbgs() << "Small/Low"; break;
      case CodeModel::Kernel: dbgs() << "Kernel"; break;
      case CodeModel::Medium: dbgs() << "Medium/Any"; break;
      case CodeModel::Large: dbgs() << "Large"; break;
    }
    dbgs() <<"] ";
    N->dump();
  );
  ...
  case CodeModel::Small: {
    ...
    return SDValue(DAG.getMachineNode(RISCV::PseudoLAI, DL, Ty, AddrHi, AddrLo), 0);
  }
...
SDValue RISCVTargetLowering::LowerINTRINSIC_VOID(SDValue Op,
  ...
  // N->getOperand(1)); SDValue NewWOp = DAG.getNode(N->getOpcode(), DL,
  // MVT::i64, NewOp0, NewOp1); SDValue NewRes =
  // DAG.getNode(ISD::SIGN_EXTEND_INREG, DL, MVT::i64, NewWOp,
  //                              DAG.getValueType(MVT::i32));
  // return DAG.getNode(ISD::TRUNCATE, DL, MVT::i32, NewRes);

  switch (IntNo) {
  default:
    llvm_unreachable("Unhandled INTRINSIC_VOID");
  ...
  //   SDValue FullPtr = Op.getOperand(2);
  //   SDValue NewNSID = Op.getOperand(3);

  //   assert(FullPtr.getOpcode() == ISD::CopyFromReg && "SET_NSID must have a register arg");

  //   Register FullPtrReg = cast<RegisterSDNode>(FullPtr.getOperand(1))->getReg();

  //   SDValue Ptr = DAG.getNode(RISCVISD::ZENO_GET_PTR, DL, MVT::i64, FullPtr);
  //   SDValue CombinePtr = DAG.getNode(RISCVISD::ZENO_COMBINE_PTR_NSID, DL, DAG.getVTList(MVT::i128,MVT::Other), Ptr, NewNSID, Chain);
  //   SDValue WriteReg = DAG.getCopyToReg(CombinePtr, DL, FullPtrReg, CombinePtr);
  ...
  //     // Op.getOperand(i)->dump();
  //   }
  //   SDValue OldOp4 = Op->getOperand(4);
  //   SDValue NewOp4 = DAG.getNode(ISD::ANY_EXTEND, DL, MVT::i64, OldOp4);
  //   SDValue NewIntrin =
  //       DAG.getNode(ISD::INTRINSIC_VOID, DL, MVT::Other,
  //                   {Op.getOperand(0), Op.getOperand(1), Op.getOperand(2),
  //                    Op.getOperand(3), NewOp4});
  //   // OldOp4.dump();
  //   // NewOp4.dump();
  //   // NewIntrin.dump();
  //   //SDValue ResultNode =  DAG.getNode(ISD::TRUNCATE, DL, OldOp4.getSimpleValueType(), NewIntrin);
  //   //ResultNode.dump();
  //   //return ResultNode;
  //   return NewIntrin;
  // }
...
static SDValue customLowerZenoIntrinLoad(SDValue Op, SelectionDAG &DAG,
                                     bool SeperateNSID) {
  SDNode *N = Op.getNode();
  assert(isa<MemIntrinsicSDNode>(N) &&
         "Intrinisc did not get marked as memory intrinsic");
  auto MMO = cast<MemIntrinsicSDNode>(N)->getMemOperand();
  SDLoc DL(N);
  ...
  if (SeperateNSID) {
    auto OrigPtr =
        DAG.getNode(RISCVISD::ZENO_GET_PTR, DL, MVT::i64, N->getOperand(2));
    auto NSID = N->getOperand(3);
    Ptr = DAG.getNode(RISCVISD::ZENO_COMBINE_PTR_NSID, DL,
                      DAG.getVTList(MVT::i128), OrigPtr, NSID);
  } else {
    Ptr = N->getOperand(2);
  }
  auto Undef = DAG.getUNDEF(Ptr.getValueType());
  auto Res = DAG.getLoad(ISD::UNINDEXED, ISD::NON_EXTLOAD, N->getValueType(0),
                         DL, Chain, Ptr, Undef, N->getValueType(0), MMO);
  return Res;
}

static SDValue customLowerZenoIntrinStore(SDValue Op, SelectionDAG &DAG,
                                     bool SeperateNSID) {
  SDNode *N = Op.getNode();
  assert(isa<MemIntrinsicSDNode>(N) &&
         "Intrinisc did not get marked as memory intrinsic");
  ...
  SDValue Val;
  if (SeperateNSID) {
    // 0 is chain, 1 is which intrin, 2 is ptr, 3 is nsid, 4 is value 
    auto OrigPtr =
        DAG.getNode(RISCVISD::ZENO_GET_PTR, DL, MVT::i64, N->getOperand(2));
    auto NSID = N->getOperand(3);
    Ptr = DAG.getNode(RISCVISD::ZENO_COMBINE_PTR_NSID, DL,
    ...
    // 0 is chain, 1 is which intrin, 2 is ptr, 3 is value 
    Ptr = N->getOperand(2);
    Val = N->getOperand(3);
  }
  auto Res = DAG.getStore(Chain, DL, Val, Ptr, MMO);
  return Res;
}
...
SDValue RISCVTargetLowering::LowerINTRINSIC_W_CHAIN(SDValue Op,
  ...
  case Intrinsic::riscv_zeno_store_64_nsid:
  case Intrinsic::riscv_zeno_store_128_nsid: {
    return customLowerZenoIntrinStore(Op, DAG, true);
  }
  ...
  case Intrinsic::riscv_zeno_store_64:
  case Intrinsic::riscv_zeno_store_128: {
    return customLowerZenoIntrinStore(Op, DAG, false);
  }
  case Intrinsic::riscv_zeno_load_64_nsid:
  case Intrinsic::riscv_zeno_load_128_nsid: {
    return customLowerZenoIntrinLoad(Op, DAG, true);
  }
  case Intrinsic::riscv_zeno_load_64:
  case Intrinsic::riscv_zeno_load_128: {
    return customLowerZenoIntrinLoad(Op, DAG, false);
  }
  }
...
static SDValue customLegalizeToWOp(SDNode *N, SelectionDAG &DAG,
  ...
  SDValue NewOp0 = DAG.getNode(ExtOpc, DL, MVT::i128, N->getOperand(0));
  SDValue NewOp1 = DAG.getNode(ExtOpc, DL, MVT::i128, N->getOperand(1));
  SDValue NewRes = DAG.getNode(WOpcode, DL, MVT::i128, NewOp0, NewOp1);
...
// handles 8, 16, 32
static SDValue customLegalizeZenoIntrin(SDNode *N, 
                                        SelectionDAG &DAG, bool SeperateNSID, ISD::LoadExtType ExtenType) {
  // LLVM_DEBUG(dbgs() << "Legalizing")
  assert(isa<MemIntrinsicSDNode>(N) &&
         "Intrinisc did not get marked as memory intrinsic");
  auto MMO = cast<MemIntrinsicSDNode>(N)->getMemOperand();
  SDLoc DL(N);
  ...
  auto Undef = DAG.getUNDEF(Ptr.getValueType());
  auto Res = DAG.getLoad(ISD::UNINDEXED, ExtenType, MVT::i64, DL,
                         Chain, Ptr, Undef, N->getValueType(0), MMO);
  return Res;
}
...
void RISCVTargetLowering::ReplaceNodeResults(SDNode *N,
  ...
  case ISD::INTRINSIC_W_CHAIN: {
    // add either a sign exrtension or a zero extension, depending on if signed
    // or unsigned
    if(N->getNumOperands() < 2) break;
    if(!isa<ConstantSDNode>(N->getOperand(1))) break;
    auto IntrinID = N->getConstantOperandAPInt(1).trunc(64).getSExtValue();
    SDValue Res;
    switch (IntrinID) {
    case Intrinsic::riscv_zeno_load_u8:
    case Intrinsic::riscv_zeno_load_u16:
      Res = customLegalizeZenoIntrin(N, DAG, false, ISD::ZEXTLOAD);
      break;
    case Intrinsic::riscv_zeno_load_s8:
    case Intrinsic::riscv_zeno_load_s16:
    case Intrinsic::riscv_zeno_load_32:
      Res = customLegalizeZenoIntrin(N, DAG, false, ISD::SEXTLOAD);
      break;
    case Intrinsic::riscv_zeno_load_u8_nsid:
    case Intrinsic::riscv_zeno_load_u16_nsid:
      Res = customLegalizeZenoIntrin(N, DAG, true, ISD::ZEXTLOAD);
      break;
    case Intrinsic::riscv_zeno_load_s16_nsid:
    case Intrinsic::riscv_zeno_load_s8_nsid:
    case Intrinsic::riscv_zeno_load_32_nsid:
      Res = customLegalizeZenoIntrin(N, DAG, true, ISD::SEXTLOAD);
      break;
    }
    if (Res->getNumValues() == 2) {
...
SDValue RISCVTargetLowering::PerformDAGCombine(SDNode *N,
  ...
  case ISD::ZERO_EXTEND:
    // if attempting to extend a value to the same type, just replace it
    if(N->getSimpleValueType(0) == N->getOperand(0).getSimpleValueType()) {
      return DCI.CombineTo(N, N->getOperand(0));
    }
...
static MachineBasicBlock *emitSplitF64Pseudo(MachineInstr &MI,
  ...
  // unsigned LoadOpcode = MF.getSubtarget<RISCVSubtarget>().hasStdExtZZeno() ? RISCV::PseudoELW : RISCV::LW;
  unsigned LoadOpcode = MF.getSubtarget<RISCVSubtarget>().hasStdExtZZeno() ? RISCV::ELW : RISCV::LW;
...
static MachineBasicBlock *emitBuildPairF64Pseudo(MachineInstr &MI,
  ...
  // unsigned StoreOpcode = MF.getSubtarget<RISCVSubtarget>().hasStdExtZZeno() ? RISCV::PseudoESW : RISCV::SW;
  unsigned StoreOpcode = MF.getSubtarget<RISCVSubtarget>().hasStdExtZZeno() ? RISCV::ESW : RISCV::SW;
...
// static MachineBasicBlock *emitExtendedZenoLoad(MachineInstr &MI,
//                                              MachineBasicBlock *BB) {
  
//   Register DstGPR = MI.getOperand(0).getReg();
//   Register SrcPseudo = MI.getOperand(1).getReg();
//   Register SrcGPR = RISCV::X0 + (SrcPseudo.id() - RISCV::PXE0);
//   int64_t Imm = MI.getOperand(2).getImm();

//   unsigned Opcode = 0;
//   switch(MI.getOpcode()) {
//     default: llvm_unreachable("Unexpected instruction");
//     case RISCV::PseudoELD:  Opcode = RISCV::ELD;  break;
//     case RISCV::PseudoELW:  Opcode = RISCV::ELW;  break;
//     case RISCV::PseudoELH:  Opcode = RISCV::ELH;  break;
//     case RISCV::PseudoELHU: Opcode = RISCV::ELHU; break;
//     case RISCV::PseudoELB:  Opcode = RISCV::ELB;  break;
//     case RISCV::PseudoELBU: Opcode = RISCV::ELBU; break;
//   }
...
//   dbgs() << "EMIT DAG: emit opcode " << Opcode << "\n";

//   // dbgs() << "nargs: " << MI.getNumOperands() << "\n";
//   // dbgs() << "inst: ";
//   // MI.dump();
...
// TODO: should we also be searching MN->op_begin()? is that helpful? for example in address management. my guess is yes
static bool isAnyPtr(SDNode *Node) {
  
  // high level: for all memory operands, get their TBAA info. then go throough all of the TBAA info (which can be pretty heavily nested) and find metadata with "any pointer"
  // TBAA: Type Based Alias Analysis

  if (auto *MN = dyn_cast<MachineSDNode>(Node)) {
    for (auto MMO = MN->memoperands_begin(); MMO != MN->memoperands_end();
         MMO++) {
      auto TBAA = (*MMO)->getAAInfo().TBAA;
      // LLVM_DEBUG(
      //   dbgs()<<"isAnyPtr::TBAA: "; 
      //   if(TBAA)
      //     TBAA->dump();
      ...
      // );
      if (TBAA) {
        for (auto MDO = TBAA->op_begin(); MDO != TBAA->op_end(); MDO++) {
          if(auto *MD = dyn_cast<MDNode>(*MDO)) {
            for (auto MDOS = MD->op_begin(); MDOS != MD->op_end(); MDOS++) {
              if (auto *MDS = dyn_cast<MDString>(*MDOS)) {
                if (MDS->getString() == "any pointer") {
                  return true;
                }
              }
            }
...
void RISCVTargetLowering::AdjustInstrPostInstrSelection(MachineInstr &MI,
  ...
  // LLVM_DEBUG(
  //   dbgs() << "RISCV::AdjustInstrPostInstrSelection: checking if its a
  //   pointer\n"; Node->dump(); MI.dump();
  // );
  ...
  case RISCV::PseudoELE:
    if (isAnyPtr(Node)) {
      // LLVM_DEBUG(dbgs()<<"RISCV::AdjustInstrPostInstrSelection: Am I a
      // pointer?\n";); MI.dump();
      MI.setDesc(TII->get(RISCV::PseudoELP));
      ...
      // MachineOperand NewMO = MI.getOperand(1);
      // NewMO.setIsKill(false);
      // NewMO.setIsDef(true);
      // // NewMO.setIsDead(true);
      // NewMO.setImplicit(true);
      // MI.addOperand(NewMO);

      // MI.dump();

      // auto RegMO = MachineOperand::CreateReg(Reg, /*def*/true, /*imp*/true,
      // /*kill*/false);

      // MI.getOperand(1).dump();
      // dbgs() << MI.getOperand(1).isDef() << " " << MI.getOperand(1).isUse()
      // << "\n";

      // MI.getOperand(0).setIsEarlyClobber(true);

      // add implict operand that is a def
      // tieOperands togteher

      // MI.dump();

      // // add the two sub regs as implict operands that are killed
      // unsigned Reg = MI.getOperand(1).getReg();
      // unsigned SubGPR = TRI->getSubReg(Reg, RISCV::sub_ext_64);
      // unsigned SubEXT = TRI->getSubReg(Reg, RISCV::sub_ext_64_64);
      // auto SubGPRMO = MachineOperand::CreateReg(SubGPR, /*def*/false,
      // /*imp*/true, /*kill*/true); auto SubEXTMO =
      // MachineOperand::CreateReg(SubEXT, /*def*/false, /*imp*/true,
      // /*kill*/true); MI.addOperand(SubGPRMO); MI.addOperand(SubEXTMO);
  ...
  case RISCV::PseudoESE:
    if (isAnyPtr(Node)) {
      MI.setDesc(TII->get(RISCV::PseudoESP));
    }
    break;
  default:
    LLVM_DEBUG(dbgs() << "adjust: "; MI.dump(););
  }
}
...
static const MCPhysReg ArgPXERs[] = {
  RISCV::PXE10, RISCV::PXE11, RISCV::PXE12, RISCV::PXE13,
  RISCV::PXE14, RISCV::PXE15, RISCV::PXE16, RISCV::PXE17
};
...
static bool CC_RISCV(const DataLayout &DL, RISCVABI::ABI ABI, unsigned ValNo,
  ...
  if (ValVT == MVT::f16 && !UseGPRForF16_F32)
    ...
    Reg = State.AllocateReg(ArgPXERs);
  }
  else {
...
static bool CC_RISCV_FastCC(const DataLayout &DL, RISCVABI::ABI ABI,
  ...
  if (LocVT == MVT::i128) {
    if (unsigned Reg = zeno::getPseudoReg(State.AllocateReg(GPRList))) {
      State.addLoc(CCValAssign::getReg(ValNo, ValVT, Reg, LocVT, LocInfo));
      return false;
    }
  }
...
#define NODE_NAME_CASE(NODE)                                                   \
  ...
  NODE_NAME_CASE(ZENO_GET_PTR)
  NODE_NAME_CASE(ZENO_SET_PTR)
  NODE_NAME_CASE(ZENO_GET_NSID)
  NODE_NAME_CASE(ZENO_SET_NSID)
  NODE_NAME_CASE(ZENO_COMBINE_PTR_NSID)
  }
  // clang-format on
  return nullptr;
...
RISCVTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
    switch (Constraint[0]) {
    case 'r':
      // FIXME(jacob-abraham) maybe make a sperate constraint for pointers?

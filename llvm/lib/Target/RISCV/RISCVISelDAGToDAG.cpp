...
#ifndef NDEBUG
static cl::opt<bool> ShowDAGBefore128Map("dag-before-128-map", cl::Hidden,
                                   cl::init(false),
                                   cl::Optional,
                                   cl::desc("Show a DAG graph before mapping"));
static cl::opt<bool> ShowDAGAfter128Map("dag-after-128-map", cl::Hidden,
                                   cl::init(false),
                                   cl::Optional,
                                   cl::desc("Show a DAG graph after mapping"));
static cl::opt<bool> ShowDAGPass128Map("dag-pass-128-map", cl::Hidden,
                                   cl::init(false),
                                   cl::Optional,
                                   cl::desc("Show a DAG graph for each mapping pass"));
static cl::opt<bool> SaveDAGToFile("dag-save", cl::Hidden,
                                   cl::init(false),
                                   cl::Optional,
                                   cl::desc("Saves DAG to file instead of viewing"));
...
static SDNode *selectImm(SelectionDAG *CurDAG, const SDLoc &DL, int64_t Imm,
                         const RISCVSubtarget &Subtarget, bool use128=false, bool isTarget=true);

// replace all values of Src with Replacement
// afterwards, Src must be empty so we can remove it
// only works when Src and Replacement ghave the same number of results
static void ReplaceAllUsesOfNodeWithNode(SelectionDAG *CurDAG,
                                         SelectionDAG::allnodes_iterator &Iter,
                                         SDNode *Src, SDNode *Replacement) {
  // backup up the iterator
  --Iter;
  // replace all values, chain, glue, etc
  // LLVM_DEBUG(dbgs() << "check results\n"; Src->dump(); Replacement->dump(););
  assert(Src->getNumValues() <= Replacement->getNumValues() &&
         "Mismatching number of values");
  for (unsigned i_res = 0; i_res < Src->getNumValues(); i_res++) {
    CurDAG->ReplaceAllUsesOfValueWith(SDValue(Src, i_res),
                                      SDValue(Replacement, i_res));
  }
  // reset the iterator
  ++Iter;
  // remove the Src, it is gone
  assert(Src->use_empty() && "Node is not empty");
  CurDAG->DeleteNode(Src);
}

// replace all values of Src with Replacement
// afterwards, Src must be empty so we can remove it
static void ReplaceAllUsesOfNodeWithValue(SelectionDAG *CurDAG,
                                          SelectionDAG::allnodes_iterator &Iter,
                                          SDNode *Src,
                                          const SDValue &Replacement) {
  // backup up the iterator
  --Iter;
  // replace all values, chain, glue, etc
  for (unsigned i_res = 0; i_res < Src->getNumValues(); i_res++) {
    CurDAG->ReplaceAllUsesOfValueWith(SDValue(Src, i_res), Replacement);
  }
  // reset the iterator
  ++Iter;
  // remove the Src, it is gone
...
// replace the single value of Src with Replacement
// afterwards, Src must be empty so we can remove it
static void ReplaceUseOfNodeWithValue(SelectionDAG *CurDAG,
                                      SelectionDAG::allnodes_iterator &Iter,
                                      SDNode *Src, const SDValue &Replacement) {
  // backup up the iterator
  --Iter;
  // replace value
  CurDAG->ReplaceAllUsesOfValueWith(SDValue(Src, 0), Replacement);
  // reset the iterator
  ++Iter;
  // remove the Src, it is gone
  assert(Src->use_empty() && "Node is not empty");
...
// constants are special, handle them seperatly and first
// essentially replace all 128 bit constants with 64 bits
// except for zeros, those MUST stay 128 bits to makes movs work right
static void Wrap128To64_FixConstants(SelectionDAG *CurDAG) {
  for (SelectionDAG::allnodes_iterator I = CurDAG->allnodes_begin(),
    ...
    bool isTgtCnst = N->getOpcode() == ISD::TargetConstant;
    if ((N->getOpcode() == ISD::Constant || isTgtCnst) && N->getValueType(0) == MVT::i128) {
      auto *ConstNode = cast<ConstantSDNode>(N);
      // dont replace 0
      if (!ConstNode->isNullValue()) {
        LLVM_DEBUG(dbgs() << "lowering constant\n";);
        SDValue New = CurDAG->getConstant(
            ConstNode->getAPIntValue().truncSSat(64).getSExtValue(), DL,
            MVT::i64, isTgtCnst);

        ReplaceUseOfNodeWithValue(CurDAG, I, N, New);
      }
    }
  }
}

// fixup instruction return type
static void Wrap128To64_FixupReturn(SelectionDAG *CurDAG, SelectionDAG::allnodes_iterator &Iter, SDNode *N) {
  assert(N->getSimpleValueType(0) == MVT::i128 && "Return type was not i128");
  SDLoc DL(N);
  ...
  SDValue New;
  // TODO(jacob-abraham) this special case should be moved out of here
  if (N->getOpcode() == ISD::TargetGlobalAddress) {
    auto *GA = cast<GlobalAddressSDNode>(N);
    New = CurDAG->getTargetGlobalAddress(GA->getGlobal(), DL, MVT::i64,
    ...
    for (unsigned i = 0; i < N->getNumValues(); i++)
      Tys.push_back(N->getValueType(i));
    Tys[0] = MVT::i64;

    New = CurDAG->getNode(N->getOpcode(), DL, CurDAG->getVTList(Tys), Ops,
                          N->getFlags());
  }

  ReplaceAllUsesOfNodeWithNode(CurDAG, Iter, N, New.getNode());
}

// fixes instructions that have been put in an invalid state
static void Wrap128To64_FixInstructions(SelectionDAG *CurDAG) {
  for (SelectionDAG::allnodes_iterator I = CurDAG->allnodes_begin(),
                                       E = CurDAG->allnodes_end();
       I != E;) {
    SDNode *N = &*I++; // Preincrement iterator to avoid invalidation issues.
    ...
    // if get_ptr is i64 to i64 its useless and causes problems, simply remove
    if ((N->getOpcode() == RISCVISD::ZENO_GET_PTR ||
         N->getOpcode() == RISCVISD::ZENO_GET_NSID) &&
        N->getSimpleValueType(0) == MVT::i64 &&
        N->getOperand(0)->getSimpleValueType(0) == MVT::i64) {
      LLVM_DEBUG(dbgs() << "Remove useless: "; N->dump(););
      ReplaceUseOfNodeWithValue(CurDAG, I, N, N->getOperand(0));
    }

    // fix truncating stores
    if (N->getOpcode() == ISD::STORE) {
      auto STN = cast<StoreSDNode>(N);
      // trunc 64->64, just mark as not truncate
      if (STN->getMemoryVT().getSimpleVT() ==
              STN->getValue().getSimpleValueType() &&
          STN->isTruncatingStore()) {
        LLVM_DEBUG(dbgs() << "unset truncating store: "; N->dump(););
        STN->setTruncatingStore(false);
      }
      // if its truncating a 128, insert a get ptr
      if (STN->getValue().getSimpleValueType() == MVT::i128 &&
          STN->getMemoryVT() != MVT::i128) {
        LLVM_DEBUG(dbgs() << "change truncating 128 to normal pointer store: "; N->dump(););
        SDValue OldValue = STN->getValue();
        SDValue NewValue =
            CurDAG->getNode(RISCVISD::ZENO_GET_PTR, DL, MVT::i64, OldValue);

        SDValue New;
        if (STN->getMemoryVT() == MVT::i64)
          New =
              CurDAG->getStore(STN->getChain(), DL, NewValue, STN->getBasePtr(),
                               STN->getPointerInfo(), STN->getAlign(),
                               STN->getMemOperand()->getFlags());
        else
          New = CurDAG->getTruncStore(STN->getChain(), DL, NewValue,
                                      STN->getBasePtr(), STN->getPointerInfo(),
                                      STN->getMemoryVT(), STN->getAlign(),
                                      STN->getMemOperand()->getFlags());

        ReplaceAllUsesOfNodeWithNode(CurDAG, I, N, New.getNode());
      }
    }

    if (N->getOpcode() == ISD::LOAD) {
      auto LDN = cast<LoadSDNode>(N);
      if (LDN->getValueType(0) == MVT::i128 &&
          (LDN->getMemoryVT() != MVT::i128 && LDN->getMemoryVT() != MVT::i64)) {
        LLVM_DEBUG(dbgs() << "fixup loads: "; N->dump(););
        SDValue New = CurDAG->getLoad(
            LDN->getAddressingMode(), LDN->getExtensionType(), MVT::i64, DL,
            LDN->getChain(), LDN->getBasePtr(), LDN->getOffset(),
            LDN->getMemoryVT(), LDN->getMemOperand());

        ReplaceAllUsesOfNodeWithNode(CurDAG, I, N, New.getNode());
      }
    }

    // if copy from reg and its 128 and copying from not a pxer, change to a
    // copy to reg class
    if (N->getOpcode() == ISD::CopyFromReg &&
        N->getSimpleValueType(0) == MVT::i128) {
      auto Reg = cast<RegisterSDNode>(N->getOperand(1))->getReg();
      // if reg is not a pxer and is physical
      // EXAMPLE: t21: i128,ch = CopyFromReg t19, Register:i128 $physreg106
      if (!RISCV::PXERRegClass.contains(Reg) && Reg.isPhysical()) {
        LLVM_DEBUG(dbgs() << "fixup register copy: "; N->dump(););
        SDValue Chain = N->getOperand(0);
        SDValue New;
        if (SDNode *GlueNode = N->getGluedNode())
          New = CurDAG->getCopyFromReg(Chain, DL, Reg, MVT::i64,
...
// fixup an operand only
static void Wrap128To64_FixupOneOperand(SelectionDAG *CurDAG, SDNode *N,
                                        unsigned OperandIdx) {
  assert(N->getSimpleValueType(0) != MVT::i128 &&
         "Return type was i128, should use other method");
  SDLoc DL(N);

  // we only need to extract the pointer, since there is no 128 bit result
  SDValue Operand = N->getOperand(OperandIdx);
  SDValue Ptr = CurDAG->getNode(RISCVISD::ZENO_GET_PTR, DL, MVT::i64, Operand);

  SmallVector<SDValue, 8> Ops;
  for (auto O : N->op_values()) {
    Ops.push_back(O);
  }
  // set the correct 64 bit operand
  Ops[OperandIdx] = Ptr;

  SDNode *Chain = CurDAG->MorphNodeTo(N, N->getOpcode(), N->getVTList(), Ops);
}

// fixup an operand and the return type
static void Wrap128To64_FixupOneOperandAndReturn(SelectionDAG *CurDAG, SelectionDAG::allnodes_iterator &Iter,
                                                 SDNode *N,
                                                 unsigned OperandIdx) {
  assert(N->getSimpleValueType(0) == MVT::i128 && "Return type was not i128");
  SDLoc DL(N);

  // we need to extract the pointer and namespace, and then recombine
  SDValue Operand = N->getOperand(OperandIdx);
  SDValue Ptr = CurDAG->getNode(RISCVISD::ZENO_GET_PTR, DL, MVT::i64, Operand);
  SDValue NSID =
      CurDAG->getNode(RISCVISD::ZENO_GET_NSID, DL, MVT::i64, Operand);

    // we need to extract the pointer, compute the new value, and then reset in NS
  // SDValue PtrAndNSID = N->getOperand(OperandIdx);
  // SDValue Ptr = CurDAG->getNode(RISCVISD::ZENO_GET_PTR, DL, MVT::i64, PtrAndNSID);
  ...
  for (unsigned i = 0; i < N->getNumValues(); i++)
    Tys.push_back(N->getValueType(i));
  // set the correct return type, now its i64
  Tys[0] = MVT::i64;

  SDValue New = CurDAG->getNode(N->getOpcode(), DL, CurDAG->getVTList(Tys), Ops,
                                N->getFlags());

  // now the result of New should be combined with the NSID
  SDValue Replacement =
      CurDAG->getNode(RISCVISD::ZENO_COMBINE_PTR_NSID, DL,
                      CurDAG->getVTList(MVT::i128), New, NSID);
  // SDValue Replacement =
  //     CurDAG->getNode(RISCVISD::ZENO_SET_PTR, DL,
  //                     CurDAG->getVTList(MVT::i128), PtrAndNSID, New);
  

  // now we can replace N
  ReplaceAllUsesOfNodeWithNode(CurDAG, Iter, N, Replacement.getNode());
}

// fixup operands and the return type
static void Wrap128To64_FixupSelectCC(SelectionDAG *CurDAG,
                                      SelectionDAG::allnodes_iterator &Iter,
                                      SDNode *N, unsigned OperandIdx1,
                                      unsigned OperandIdx2) {
  assert(N->getSimpleValueType(0) == MVT::i128 && "Return type was not i128");
  assert(N->getOpcode() == RISCVISD::SELECT_CC ||
         N->getOpcode() == ISD::SETCC &&
             "This function is only for RISCVISD::SELECT_CC or ISD::SETCC");
  SDLoc DL(N);

  // we need to extract the pointer and namespace, and then recombine
  SDValue Operand1 = N->getOperand(OperandIdx1);
  SDValue Ptr1 =
      CurDAG->getNode(RISCVISD::ZENO_GET_PTR, DL, MVT::i64, Operand1);
  SDValue NSID1 =
      CurDAG->getNode(RISCVISD::ZENO_GET_NSID, DL, MVT::i64, Operand1);

  SDValue Operand2 = N->getOperand(OperandIdx2);
  SDValue Ptr2 =
      CurDAG->getNode(RISCVISD::ZENO_GET_PTR, DL, MVT::i64, Operand2);
  SDValue NSID2 =
      CurDAG->getNode(RISCVISD::ZENO_GET_NSID, DL, MVT::i64, Operand2);

  // this is truly awful code but we need 2 selects, one for the ptr and one for
  // the ns then the two selects get put in the combine

  SmallVector<SDValue, 8> OpsPtr;
  SmallVector<SDValue, 8> OpsNSID;
  for (auto O : N->op_values()) {
    OpsPtr.push_back(O);
    OpsNSID.push_back(O);
  }
  ...
  // set the correct 64 bit operand
  OpsPtr[OperandIdx1] = Ptr1;
  OpsPtr[OperandIdx2] = Ptr2;
  OpsNSID[OperandIdx1] = NSID1;
  OpsNSID[OperandIdx2] = NSID2;
  ...
  SDValue NewPtr = CurDAG->getNode(N->getOpcode(), DL, CurDAG->getVTList(Tys),
                                   OpsPtr, N->getFlags());

  SDValue NewNSID = CurDAG->getNode(N->getOpcode(), DL, CurDAG->getVTList(Tys),
                                    OpsPtr, N->getFlags());

  // now the result of New should be combined with the NSID
  SDValue Replacement =
      CurDAG->getNode(RISCVISD::ZENO_COMBINE_PTR_NSID, DL,
                      CurDAG->getVTList(MVT::i128), NewPtr, NewNSID);

  // now we can replace N
  ReplaceAllUsesOfNodeWithNode(CurDAG, Iter, N, Replacement.getNode());
}

static void Wrap128To64(SelectionDAG *CurDAG) {
  LLVM_DEBUG(dbgs() << "preprocess dag 128 wrapping\n";
             dbgs() << "============\n"; CurDAG->dump();
             dbgs() << "============\n";);

  #ifndef NDEBUG
   MachineFunction &MF = CurDAG->getMachineFunction();
    std::string BlockName = (MF.getName() + ":" + CurDAG->getFunctionLoweringInfo()->MBB->getBasicBlock()->getName()).str();
  if(ShowDAGBefore128Map) {
    std::string Title = "before 128 mapping " + BlockName;
    if(SaveDAGToFile)
      CurDAG->dumpDotGraph(".before128map.dot", Title);
    else
      CurDAG->viewGraph(Title);
  }
  #endif

  // should be done first
  Wrap128To64_FixConstants(CurDAG);

  // fix instructions before wrapping
  Wrap128To64_FixInstructions(CurDAG);
  ...
  for (SelectionDAG::allnodes_iterator I = CurDAG->allnodes_begin(),
    ...
    // leave memory ops alone
    if (N->getOpcode() == ISD::LOAD || N->getOpcode() == ISD::STORE)
      continue;
    // leave functions calls alone
    if (N->getOpcode() == RISCVISD::CALL ||
        N->getOpcode() == ISD::CALLSEQ_START ||
        N->getOpcode() == ISD::CALLSEQ_END)
      continue;
    // live inline asm alone
    if (N->getOpcode() == ISD::INLINEASM || N->getOpcode() == ISD::INLINEASM_BR)
      continue;
    // leave registers alone
    if (N->getOpcode() == ISD::CopyFromReg ||
        N->getOpcode() == ISD::CopyToReg || N->getOpcode() == ISD::Register)
      continue;
    // leave zeno NS stuff alone
    if (N->getOpcode() == RISCVISD::ZENO_GET_PTR ||
        N->getOpcode() == RISCVISD::ZENO_SET_PTR ||
        N->getOpcode() == RISCVISD::ZENO_GET_NSID ||
        N->getOpcode() == RISCVISD::ZENO_SET_NSID ||
        N->getOpcode() == RISCVISD::ZENO_COMBINE_PTR_NSID)
      continue;
    // leave constants alone, they have already been handled
    if (N->getOpcode() == ISD::Constant ||
        N->getOpcode() == ISD::TargetConstant ||
        N->getOpcode() == ISD::FrameIndex)
      continue;
    if(N->getOpcode() == ISD::TargetJumpTable) continue;
    if(N->getOpcode() == ISD::INTRINSIC_W_CHAIN) continue;
    if(N->getOpcode() == ISD::LIFETIME_START || N->getOpcode() == ISD::LIFETIME_END) continue;
    if(N->getOpcode() == ISD::AssertZext || N->getOpcode() == ISD::AssertSext) continue;
    
    // skip target specific stuff
    // if(N->isTargetOpcode()) continue;
    
    // skip machine nodes
    if(N->isMachineOpcode()) continue;

    // if node doesn't have any operands, leave alone
    if (N->use_empty())
      continue;
    // if node doesn't produce any values
    if (N->getNumValues() < 1)
      continue;
    ...
    SmallSet<size_t, 6> ss;
    for (size_t idx = 0; idx < N->getNumOperands(); idx++) {
      const auto &V = N->getOperand(idx);
      if (V.getResNo() < V.getNode()->getNumValues() &&
          V.getSimpleValueType() == MVT::i128) {
        ss.insert(idx);
      }
    }

    //if we only need to lower the return type, thats easy
    if (VT == MVT::i128 && ss.empty()) {
      LLVM_DEBUG(dbgs() << "lowering simple return: "; N->dump(););
      Wrap128To64_FixupReturn(CurDAG, I, N);
    }
    // if we need to lower 1 operand and the return type,  handle it
    else if (VT == MVT::i128 && ss.size() == 1) {
      LLVM_DEBUG(dbgs() << "lowering 1 operand and return type: "; N->dump(););
      Wrap128To64_FixupOneOperandAndReturn(CurDAG, I, N, *ss.begin());
    }
    // if we need to lower 1 operand and not the return type, handle it
    else if (VT != MVT::i128 && ss.size() == 1) {
      LLVM_DEBUG(dbgs() << "lowering 1 operand: "; N->dump(););
      Wrap128To64_FixupOneOperand(CurDAG, N, *ss.begin());
    }
    // special case for select
    else if (VT == MVT::i128 && (ss.size() == 3 || ss.size() == 2) &&
             (N->getOpcode() == RISCVISD::SELECT_CC ||
              N->getOpcode() == ISD::SETCC)) {
      LLVM_DEBUG(dbgs() << "lowering SELECT_CC/SETCC operands: "; N->dump(););

      // FIXME(jacob-abraham) this special case can probably be generalized for
      // the 2 pointer operand case? make the prehaps poor assumption that these
      // two idx are the truvv and falsev unsigned Idx1 = *ss.begin(); unsigned
      // Idx2 = *(std::next(ss.begin()));

      unsigned Idx1;
      unsigned Idx2;
      if (N->getOpcode() == RISCVISD::SELECT_CC) {
        Idx1 = 3;
        Idx2 = 4;
        ...
        llvm_unreachable("Invalid SELECT node");
      }
      Wrap128To64_FixupSelectCC(CurDAG, I, N, Idx1, Idx2);
    }
    // if return type is 128 or there are 128 bit operands and one iof the above
    // cases didnt match, warn the user
    else if (VT == MVT::i128 || !ss.empty()) {
      errs() << "WARNING: Failed to lower: '";
      N->print(errs(), nullptr);
      errs() << "'\nThis may be fine or it may be broken\n";
    }
  }
  }

  // this is a final fixup, goes last
  Wrap128To64_FixInstructions(CurDAG);

  LLVM_DEBUG(dbgs() << "after preprocess dag 128 wrapping\n";
             dbgs() << "============\n"; CurDAG->dump();
             dbgs() << "============\n";);
  
  #ifndef NDEBUG
  if(ShowDAGAfter128Map) {
    std::string Title = "after 128 mapping " + BlockName;
    if(SaveDAGToFile)
      CurDAG->dumpDotGraph(".after128map.dot", Title);
    else
      CurDAG->viewGraph(Title);
  }
  #endif
...
static SDNode *selectImm(SelectionDAG *CurDAG, const SDLoc &DL, int64_t Imm,
                         const RISCVSubtarget &Subtarget, bool use128, bool isTarget) {
  MVT XLenVT = Subtarget.getXLenVT();
  RISCVMatInt::InstSeq Seq =
      RISCVMatInt::generateInstSeq(Imm, Subtarget.getFeatureBits());

  SDNode *Result = nullptr;
  SDValue SrcReg = CurDAG->getRegister(use128 ? RISCV::PXE0 : RISCV::X0, XLenVT);
  for (RISCVMatInt::Inst &Inst : Seq) {
    SDValue SDImm = CurDAG->getConstant(Inst.Imm, DL, XLenVT, isTarget);
    if (Inst.Opc == RISCV::LUI)
      Result = CurDAG->getMachineNode(RISCV::LUI, DL, XLenVT, SDImm);
    else if (Inst.Opc == RISCV::ADDUW)
      Result = CurDAG->getMachineNode(RISCV::ADDUW, DL, XLenVT, SrcReg,
                                      CurDAG->getRegister(use128 ? RISCV::PXE0 : RISCV::X0, XLenVT));
    else
...
void RISCVDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::Constant: {
    ...
    if (ConstNode->isNullValue()) {
      if (VT == XLenVT || VT == MVT::i128) {
        SDValue New = CurDAG->getCopyFromReg(
            CurDAG->getEntryNode(), DL,
            VT == MVT::i128 ? RISCV::PXE0 : RISCV::X0, VT == MVT::i128 ? MVT::i128 : XLenVT);
        ReplaceNode(Node, New.getNode());
        return;
      }
    }
    if(VT == MVT::i128) {
      // SDValue New = CurDAG->getConstant(ConstNode->getSExtValue(), DL, MVT::i64);
      SDNode* New = selectImm(CurDAG, DL, ConstNode->getAPIntValue().truncSSat(64).getSExtValue(), *Subtarget, true/*use128*/);
      //ReplaceNode(Node, New.getNode());
      CurDAG->ReplaceAllUsesWith(SDValue(Node, 0), SDValue(New, 0));
      CurDAG->RemoveDeadNode(Node);
      return;
    }
    ReplaceNode(Node,
  ...
  case ISD::TRUNCATE: {
    // we only handle this case if Zeno
    if (!Subtarget->hasStdExtZZeno()) break;

    //if truncate 128->64, simple register copy
    if(Node->getValueType(0).getSimpleVT() == MVT::i64 && Node->getOperand(0).getValueType().getSimpleVT() == MVT::i128) {

      SDValue Value = Node->getOperand(0);
      SDValue RC = CurDAG->getTargetConstant(RISCV::PXERRegClassID, DL, XLenVT);
      SDNode *NewNode =
          CurDAG->getMachineNode(TargetOpcode::COPY_TO_REGCLASS, DL, VT, Value, RC);
  ...
  case ISD::ZERO_EXTEND: {
    ...
    //if extend 64->128, simple register copy
    if(Node->getValueType(0).getSimpleVT() == MVT::i128 && Node->getOperand(0).getValueType().getSimpleVT() == MVT::i64) {
      SDValue Value = Node->getOperand(0);
      SDValue RC = CurDAG->getTargetConstant(RISCV::PXERRegClassID, DL, XLenVT);
      SDNode *NewNode =
          CurDAG->getMachineNode(TargetOpcode::COPY_TO_REGCLASS, DL, VT, Value, RC);
  ...
  case ISD::INTRINSIC_W_CHAIN: {
    ...
    default:
      break;
    case Intrinsic::riscv_zeno_set_nsid: {
      SDValue Chain = Node->getOperand(0);
      SDValue FullPtr = Node->getOperand(2);
      SDValue NewNSID = Node->getOperand(3);

      // could also be implemented with one node, ZENO_SET_NSID

      SDNode *Ptr = CurDAG->getMachineNode(
          RISCV::PseudoZenoGetPtr, DL, CurDAG->getVTList(MVT::i64), {FullPtr});

      SDNode *CombinePtr =
          CurDAG->getMachineNode(RISCV::PseudoZenoCombinePtrNSID, DL,
                                 CurDAG->getVTList(MVT::i128, MVT::Other),
                                 {SDValue(Ptr, 0), NewNSID, Chain});

      ReplaceNode(Node, CombinePtr);
      return;
    }
    case Intrinsic::riscv_zeno_get_nsid: {
      SDValue Chain = Node->getOperand(0);
      SDValue FullPtr = Node->getOperand(2);

      SDNode *NSID = CurDAG->getMachineNode(
          RISCV::PseudoZenoGetNSID, DL, CurDAG->getVTList(MVT::i64, MVT::Other), {FullPtr, Chain});


      ReplaceNode(Node, NSID);
      return;
    }
  ...
  case ISD::INTRINSIC_VOID: {
    ...
    // case Intrinsic::riscv_zeno_store: {

    //   auto Chain = Node->getOperand(0);
    //   auto Ptr = Node->getOperand(2);
    //   auto Value = Node->getOperand(4);

    //   unsigned regId = cast<ConstantSDNode>(Node->getOperand(3))->getZExtValue();
    //   assert(regId >= 0 && regId < 32);
    //   //auto EReg = CurDAG->getRegister(RISCV::E0 + regId, MVT::i64);
    //   auto Ops = {Value, Ptr, CurDAG->getTargetConstant(0, DL, MVT::i32), CurDAG->getTargetConstant(regId, DL, MVT::i32), Chain};
    //   auto VTs = CurDAG->getVTList(MVT::Other); 

    //   unsigned Opcode = RISCV::PseudoESD; // TODO(jacob-abraham): deduce type

    //   CurDAG->SelectNodeTo(Node, 
    //                         Opcode, 
    //                         VTs,
    //                         Ops);
    ...
    //   return;
    // }
    // case Intrinsic::riscv_zeno_set_ext: {
    //   auto Chain = Node->getOperand(0);
    //   auto Value = Node->getOperand(3);

    //   unsigned regId = cast<ConstantSDNode>(Node->getOperand(2))->getZExtValue();
    //   assert(regId >= 0 && regId < 32);
    //   // TODO(jacob-abraham) might want to revist, store this extra reg same way as load/store
    //   auto EReg = CurDAG->getRegister(RISCV::E0 + regId, MVT::i64);
      
    //   auto Ops = {EReg, Value, CurDAG->getTargetConstant(0, DL, MVT::i32), Chain};
    //   auto VTs = CurDAG->getVTList(/*MVT::i64,*/ MVT::Other); // ret val, chain

    //   // use pseudo instead of regular so we can specify reg
    //   auto SetExt = CurDAG->SelectNodeTo(Node, 
    //                                   RISCV::PseudoEADDIE, 
    //                                   VTs,
    //                                   Ops);
    //   return;

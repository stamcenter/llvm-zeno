...
class RISCVAsmParser : public MCTargetAsmParser {
  ...
  // Helper to emit pseudo instruction "lai"
  // void emitLoadAddressImmediate(MCInst &Inst, SMLoc IDLoc, MCStreamer &Out);
...
bool RISCVAsmParser::parseDirectiveAttribute() {
  ...
  if (Tag == RISCVAttrs::ARCH) {
    ...
    clearFeatureBits(RISCV::FeatureStdExtZZeno, "experimental-zzeno");
    ...
    while (!Arch.empty()) {
      ...
      if (Arch[0] == 'i')
        ...
        if (Ext == "zzeno")
          setFeatureBits(RISCV::FeatureStdExtZZeno, "experimental-zzeno");
        if (Ext == "zba")
  ...
  if (IsIntegerValue)
    ...
    if (Tag != RISCVAttrs::ARCH) {
      ...
      if (getFeatureBits(RISCV::FeatureStdExtZZeno))
        formalArchStr = (Twine(formalArchStr) + "_zzeno0p1").str();
...
// void RISCVAsmParser::emitLoadAddressImmediate(MCInst &Inst, SMLoc IDLoc,
//                                           MCStreamer &Out) {
...
//   const MCExpr *SymbolHi = Inst.getOperand(1).getExpr();
//   const MCExpr *SymbolLo = Inst.getOperand(1).getExpr();
...
bool RISCVAsmParser::processInstruction(MCInst &Inst, SMLoc IDLoc,
  ...
  //   emitLoadAddressImmediate(Inst, IDLoc, Out);
  case RISCV::PseudoLA:
  ...
  //   emitLoadStoreSymbol(Inst, RISCV::ELD, IDLoc, Out, /*HasTmpReg=*/true);
  //   return false;
  // case RISCV::PseudoELW: {
  //   MCRegister Dst = Inst.getOperand(0).getReg(); // gpr
  //   MCRegister PSrc = Inst.getOperand(1).getReg(); //pext
  //   unsigned p_id = PSrc.id() - RISCV::PXE0;
  //   MCRegister Src = RISCV::E0 + p_id; //pext
  //   int64_t Imm = Inst.getOperand(2).getImm();
  //   emitToStreamer(Out, MCInstBuilder(RISCV::ELW)
  //                             .addReg(Dst)
  //                             .addReg(Src)
  //                             .addImm(Imm));
  //   return false;

...
static bool checkArgCount(Sema &S, CallExpr *call, unsigned desiredArgCountMin, unsigned desiredArgCountMax) {
  unsigned argCount = call->getNumArgs();
  if (argCount >= desiredArgCountMin && argCount <= desiredArgCountMax) return false;

  if (argCount < desiredArgCountMin)
    return S.Diag(call->getEndLoc(), diag::err_typecheck_call_too_few_args)
           << 0 /*function call*/ << desiredArgCountMin << argCount
           << call->getSourceRange();

  // Highlight all the excess arguments.
  SourceRange range(call->getArg(desiredArgCountMax)->getBeginLoc(),
                    call->getArg(argCount - 1)->getEndLoc());

  return S.Diag(range.getBegin(), diag::err_typecheck_call_too_many_args)
    << 0 /*function call*/ << desiredArgCountMax << argCount
    << call->getArg(1)->getSourceRange();
}
...
bool Sema::SemaBuiltinRISCVZenoLSCall(unsigned BuiltinID, CallExpr *TheCall) {
  if (BuiltinID == RISCV::BI__builtin_riscv_zeno_load) {
    // can pass either just ptr or ptr + nsid
    if (checkArgCount(*this, TheCall, 1, 2))
      return true;
    ...
    const PointerType *pointerType =
        FirstArg.get()->getType()->getAs<PointerType>();
    if (!pointerType) {
      return Diag(TheCall->getBeginLoc(),
                  diag::err_riscv_zeno_builtin_type_error)
             << "first"
             << "be a pointer" << (QualType)FirstArg.get()->getType()
             << Arg0->getSourceRange();
      return true;
    }

    // must be a pointer to an int or  pointer
    QualType PointeeType = pointerType->getPointeeType();
    if (!PointeeType->isIntegerType() && !PointeeType->isAnyPointerType() &&
        !PointeeType->isBlockPointerType()) {
      return Diag(TheCall->getBeginLoc(),
                  diag::err_riscv_zeno_builtin_type_error)
             << "first"
             << "point to an integer/pointer" << PointeeType
             << Arg0->getSourceRange();
      return true;
    }
    ...
    if (TheCall->getNumArgs() == 2) {
      ExprResult SecArg = DefaultLvalueConversion(TheCall->getArg(1));
      if (SecArg.isInvalid())
        return true;
      // must be int
      auto Ty = SecArg.get()->getType();
      if (!Ty->isIntegerType()) {
        return Diag(TheCall->getBeginLoc(),
                    diag::err_riscv_zeno_builtin_type_error)
               << "second"
               << "be an integer" << Ty << Arg0->getSourceRange();
        return true;
      }
      // insert implicit cast to 64 bits
      Expr *SecArgCasted =
          (Ty->isUnsignedIntegerType())
              ? ImpCastExprToType(SecArg.get(), Context.UnsignedLongLongTy,
                                  CK_IntegralCast)
                    .get()
              : ImpCastExprToType(SecArg.get(), Context.LongLongTy,
                                  CK_IntegralCast)
                    .get();
      TheCall->setArg(1, SecArgCasted);
    }

    // return type is pointeeType
    TheCall->setType(PointeeType);

    return false;
  } else if (BuiltinID == RISCV::BI__builtin_riscv_zeno_store) {
    // can pass either just ptr + value or ptr + nsid + value
    if (checkArgCount(*this, TheCall, 2, 3))
      return true;
    ...
    // TODO: HANDLE THE VALUE OR NSID
    auto ValueIDX = TheCall->getNumArgs() - 1;
    // just have a value to handle
    // type must match the pointee type
    ExprResult ValueArg = DefaultLvalueConversion(TheCall->getArg(ValueIDX));
    if (ValueArg.isInvalid())
      return true;
    auto Ty = ValueArg.get()->getType();
    if (Ty != PointeeType) {
      return Diag(TheCall->getBeginLoc(),
                  diag::err_riscv_zeno_builtin_type_error)
             << "value"
             << "match the pointer's type" << Ty << Arg0->getSourceRange();
      return true;
    }
    TheCall->setArg(ValueIDX, ValueArg.get());

    if (TheCall->getNumArgs() == 3) {
      ExprResult NSIDArg = DefaultLvalueConversion(TheCall->getArg(1));
      if (NSIDArg.isInvalid())
        return true;
      // must be int
      auto Ty = NSIDArg.get()->getType();
      if (!Ty->isIntegerType()) {
        return Diag(TheCall->getBeginLoc(),
                    diag::err_riscv_zeno_builtin_type_error)
               << "second"
      ...
      Expr *NSIDArgCasted =
          (Ty->isUnsignedIntegerType())
              ? ImpCastExprToType(NSIDArg.get(), Context.UnsignedLongLongTy,
                                  CK_IntegralCast)
                    .get()
              : ImpCastExprToType(NSIDArg.get(), Context.LongLongTy,
                                  CK_IntegralCast)
                    .get();
      TheCall->setArg(1, NSIDArgCasted);
    }
...
bool Sema::CheckRISCVBuiltinFunctionCall(const TargetInfo &TI,
  ...
  case RISCV::BI__builtin_riscv_zeno_load:
    return SemaBuiltinRISCVZenoLSCall(BuiltinID, TheCall);

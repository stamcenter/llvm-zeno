...
Value *CodeGenFunction::EmitRISCVBuiltinExpr(unsigned BuiltinID,
  ...
  Value* CustomEmitIntrin = nullptr;
  ...
  case RISCV::BI__builtin_riscv_zeno_get_nsid: {
    ID = Intrinsic::riscv_zeno_get_nsid;
    IntrinsicTypes = {};
    break;
  }
  case RISCV::BI__builtin_riscv_zeno_set_nsid: {
    ID = Intrinsic::riscv_zeno_set_nsid;
    IntrinsicTypes = {};
    break;
  }
  case RISCV::BI__builtin_riscv_zeno_load: {
    if (Ops.size() != 1 && Ops.size() != 2) {
      CGM.Error(E->getExprLoc(), "builtin takes either 1 or 2 arguments");
      return nullptr;
    }
    bool IsExplicitNSID = Ops.size() == 2;

    auto PtrType = Ops[0]->getType();
    if(!isa<llvm::PointerType>(PtrType)) {
      CGM.Error(E->getExprLoc(), "first argument to builtin must be a pointer");
      return nullptr;
    }
    bool IsUnsigned = E->getArg(0)->getType()->getPointeeType()->isUnsignedIntegerType();
    if (IsExplicitNSID && !isa<IntegerType>(Ops[1]->getType())) {
      CGM.Error(E->getExprLoc(),
                "second argument to builtin must be an integer");
      return nullptr;
    }
    if (IsExplicitNSID && Ops[1]->getType()->getPrimitiveSizeInBits() != 64) {
      CGM.Error(E->getExprLoc(),
                "second argument to builtin must be a 64-bit integer");
      return nullptr;
    }

#define GET_UNSIGNED_NSID_VERSION_(un, si)                                     \
  (IsExplicitNSID ? (IsUnsigned ? Intrinsic::riscv_zeno_load_##un##_nsid       \
                                : Intrinsic::riscv_zeno_load_##si##_nsid)      \
                  : (IsUnsigned ? Intrinsic::riscv_zeno_load_##un              \
                                : Intrinsic::riscv_zeno_load_##si))
    IntrinsicTypes = {};
   
    if (cast<llvm::PointerType>(PtrType)->hasSameElementTypeAs(
            llvm::Type::getInt8PtrTy(CGM.getLLVMContext()))) {
      ID = GET_UNSIGNED_NSID_VERSION_(u8, s8);
    } else if (cast<llvm::PointerType>(PtrType)->hasSameElementTypeAs(
                   llvm::Type::getInt16PtrTy(CGM.getLLVMContext()))) {
      ID = GET_UNSIGNED_NSID_VERSION_(u16, s16);
    } else if (cast<llvm::PointerType>(PtrType)->hasSameElementTypeAs(
                   llvm::Type::getInt32PtrTy(CGM.getLLVMContext()))) {
      ID = GET_UNSIGNED_NSID_VERSION_(32, 32);
    } else if (cast<llvm::PointerType>(PtrType)->hasSameElementTypeAs(
                   llvm::Type::getInt64PtrTy(CGM.getLLVMContext()))) {
      ID = GET_UNSIGNED_NSID_VERSION_(64, 64);
    } else if (!cast<llvm::PointerType>(PtrType)->isOpaque() &&
               isa<llvm::PointerType>(
                   cast<llvm::PointerType>(PtrType)->getElementType())) {
      ID = GET_UNSIGNED_NSID_VERSION_(128, 128);
      // need to bit cast pointer types
      Ops[0] = Builder.CreateBitCast(
          Ops[0], llvm::Type::getInt8PtrTy(CGM.getLLVMContext()));
      // get the intrinsic
      llvm::Function *F = CGM.getIntrinsic(ID);
      Value *Intrin = Builder.CreateCall(F, Ops);
      // cast back to oriignal ptr type
      // by setting CustiomEmit, we ski the ending getIntrinsic
      CustomEmitIntrin =
          Builder.CreateBitCast(Intrin, PtrType->getPointerElementType());
    }
#undef GET_UNSIGNED_NSID_VERSION_

    if (ID == Intrinsic::not_intrinsic) {
      CGM.Error(E->getExprLoc(),
                "builtin was supplied unsupported pointer type");
      return nullptr;
    }
    break;
  }
  case RISCV::BI__builtin_riscv_zeno_store: {
    if (Ops.size() != 2 && Ops.size() != 3) {
      CGM.Error(E->getExprLoc(), "builtin takes either 2 or 4 arguments");
      return nullptr;
    }
    bool IsExplicitNSID = Ops.size() == 3;
    unsigned ValueIDX = Ops.size() - 1;
...
#define GET_NSID_VERSION_(v)                                          \
  (IsExplicitNSID ? Intrinsic::riscv_zeno_store_##v##_nsid                     \
                  : Intrinsic::riscv_zeno_store_##v)
    IntrinsicTypes = {};

    if (cast<llvm::PointerType>(PtrType)->hasSameElementTypeAs(
            llvm::Type::getInt8PtrTy(CGM.getLLVMContext()))) {
      ID = GET_NSID_VERSION_(8);
    } else if (cast<llvm::PointerType>(PtrType)->hasSameElementTypeAs(
                   llvm::Type::getInt16PtrTy(CGM.getLLVMContext()))) {
      ID = GET_NSID_VERSION_(16);
    } else if (cast<llvm::PointerType>(PtrType)->hasSameElementTypeAs(
                   llvm::Type::getInt32PtrTy(CGM.getLLVMContext()))) {
      ID = GET_NSID_VERSION_(32);
    } else if (cast<llvm::PointerType>(PtrType)->hasSameElementTypeAs(
                   llvm::Type::getInt64PtrTy(CGM.getLLVMContext()))) {
      ID = GET_NSID_VERSION_(64);
    } else if (!cast<llvm::PointerType>(PtrType)->isOpaque() &&
               isa<llvm::PointerType>(
                   cast<llvm::PointerType>(PtrType)->getElementType())) {
      ID = GET_NSID_VERSION_(128);
      // need to bit cast pointer types
      Ops[0] = Builder.CreateBitCast(
          Ops[0], llvm::Type::getInt8PtrTy(CGM.getLLVMContext()));
      // need to also bit cast the value
      Ops[ValueIDX] = Builder.CreateBitCast(
          Ops[ValueIDX], llvm::Type::getInt8PtrTy(CGM.getLLVMContext()));
      // // get the intrinsic
      // llvm::Function *F = CGM.getIntrinsic(ID);
      // Value *Intrin = Builder.CreateCall(F, Ops);
      // // cast back to oriignal ptr type
      // // by setting CustiomEmit, we skip the ending getIntrinsic
      // CustomEmitIntrin =
      //     Builder.CreateBitCast(Intrin, PtrType->getPointerElementType());
    }
#undef GET_NSID_VERSION_
...
#include "clang/Basic/riscv_vector_builtin_cg.inc"
  ...
  // allow intrinsics to do custom lowering
  if(CustomEmitIntrin) return CustomEmitIntrin;

  assert(ID != Intrinsic::not_intrinsic);
  llvm::Function *F;
  F = CGM.getIntrinsic(ID, IntrinsicTypes);
  return Builder.CreateCall(F, Ops, "");
}

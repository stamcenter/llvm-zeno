...
static void getExtensionFeatures(const Driver &D,
  ...
  for (auto Ext : AllExts) {
    ...
    if (Ext == "zvlsseg") {
      ...
      Features.push_back(Args.MakeArgString("+experimental-zzeno"));
...
static bool getArchFeatures(const Driver &D, StringRef MArch,
  ...
  getExtensionFeatures(D, Args, CmdArgs, Features, MArch, OtherExts);
...
void riscv::getRISCVTargetFeatures(const Driver &D, const llvm::Triple &Triple,
  ...
  if (!getArchFeatures(D, MArch, Features, Args, CmdArgs))
    return;

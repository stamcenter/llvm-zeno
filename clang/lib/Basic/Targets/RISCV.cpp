...
void RISCVTargetInfo::getTargetDefines(const LangOptions &Opts,
  ...
  if (HasZZeno) 
    Builder.defineMacro("__riscv_zzeno", "1");
...
bool RISCVTargetInfo::hasFeature(StringRef Feature) const {
  ...
  return llvm::StringSwitch<bool>(Feature)
      ...
      .Case("experimental-zzeno", HasZZeno)
...
bool RISCVTargetInfo::handleTargetFeatures(std::vector<std::string> &Features,
  ...
  for (const auto &Feature : Features) {
    if (Feature == "+m")
      ...
      HasV = true;
    else if (Feature == "+experimental-zzeno") {
      HasZZeno = true;
      PointerWidth = 128;
      PointerAlign = 128;
      IntPtrType = SignedIntNS;
      PtrDiffType = SignedIntNS;
      resetDataLayout("e-m:e-p:128:128-i64:64-i128:128-n64-S128");
    }
